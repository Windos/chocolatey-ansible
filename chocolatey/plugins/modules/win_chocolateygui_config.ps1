...
if ($state -eq "present") {
    if ([string]::IsNullOrEmpty($value)) {
        $message = "Cannot set Chocolatey GUI config as an empty string when state=present, use state=absent instead"
...
$chocoCommand = Get-ChocolateyCommand -Executable 'chocolateyguicli.exe'
$config = Get-ChocolateyGuiConfig -ChocoCommand $chocoCommand

if ($name -notin $config.Keys) {
    $message = "The Chocolatey GUI config '{0}' is not an existing config value, check the spelling. Valid config names: {1}" -f @(
        $name
        $config.Keys -join ', '
    )
...
elseif ($state -eq "present" -and $value -ne $config.$name) {
    # chocolateyguicli.exe config set is not case sensitive, it won't make a change
    # if the value is the same but doesn't match, so we skip setting it as well in
    # that case.
